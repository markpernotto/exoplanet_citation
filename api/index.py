...
# NOTE: api.host_xyz and api.starfield imports temporarily disabled
# pending a Phase 2 deploy strategy — those modules require numpy,
# pandas, pyarrow, and pillow, which aren't in api/requirements.txt yet
# (and pushing them straight in risks tripping the Vercel hobby tier
# 50MB function size limit). Re-enable along with the requirements.txt
# update once we have a tested-and-deployed size budget.
...
# from api.starfield import load_catalog, render_png   # see note above
...
# Endpoint temporarily disabled pending deploy strategy. See note at top of
# this file about the missing requirements.txt entries for numpy/pandas/
# pyarrow/pillow. The full implementation lives in api/starfield.py and
# api/host_xyz.py; gating the route registration here means importing this
# module no longer transitively imports the heavy deps. Re-enable by
# uncommenting both the imports and the @app.get block once the deploy
# size budget is sorted.
